import string
""" Contains the implementation of the HashTable with quadratic probing and its support functions to create 
      ...
      if(self.table[insertspot] == None):
         ...
         if(self.table[insertspot].word == key and item not in self.table[insertspot].lines): 
            ...
            while(self.table[spot] != None):
               spot += (base)**2
               spot = spot%self.table_size
               base+=1

            #insert at the empty spot found from quadratic probing
            lines = []
            lines.append(item)
